...
# The YaRN black-box bench matrix lives in the sibling ninfer-4090d-bench checkout; results are
# written under its profiles/bench/<timestamp>/.
BENCH_DIR="${NINFER_BENCH_DIR:-$REPO_ROOT/../ninfer-4090d-bench}"
MODE="${1:-niah}"
cd "$BENCH_DIR" || { echo "bench checkout not found: $BENCH_DIR (set NINFER_BENCH_DIR)" >&2; exit 1; }
