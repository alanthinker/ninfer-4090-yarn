...
GenerationService::GenerationService(ServeOptions options, StartupObserver startup_observer,
    ...
    automatic_first_anchor_spacing_ =
        resolve_first_anchor_spacing(options_, engine_->options().context_cache);
...
PreparedRequest GenerationService::prepare_impl(const GenerationRequest& request,
    ...
    try {
        ...
        input.context_cache.automatic_first_anchor_spacing =
            cache_participation == CacheParticipation::ReadWrite ? automatic_first_anchor_spacing_ : 0U;
...
GenerationOutcome GenerationService::run(PreparedRequest& prepared, const StreamSink* sink,
    ...
    outcome.abandoned_prefix_detail   = std::move(result.abandoned_prefix_detail);
