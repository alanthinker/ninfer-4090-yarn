...
std::string serve_usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "       --prefer-index-adopt offers the recovery index as a normal reuse candidate\n"
           ...
           "       --rope-scaling-factor enables YaRN linear RoPE position scaling to extend "
           "context beyond the native trained length; 1.0 (default) disables it. Positions above "
           "--rope-scaling-original-context (default 262144) are compressed as "
           "original + (position - original)/factor, mapping e.g. 262144 native tokens onto ~555k "
           "positions at factor 2.12. KV cache positions are never scaled, and --max-context "
           "still bounds the physical context.\n";
}
...
ServeOptions parse_serve_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        } else if (arg == "--prefer-index-adopt") {
            options.context_cache.prefer_index_adopt = true;
