...
private:
    ...
    // Linear YaRN ramp for a single RoPE position: identity at or below the original context,
    // otherwise orig + round((p - orig) / factor). A no-op when scaling is disabled.
    [[nodiscard]] std::int32_t scale_rope_pos(std::int32_t position) const noexcept;
