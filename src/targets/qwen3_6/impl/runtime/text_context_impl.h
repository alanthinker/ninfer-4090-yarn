...
TextContext::prefill_impl(std::span<const int> ids, const TextPrefill* text_prefill,
    ...
    for (; t0 < T;) {
...

        {
            ...
            // YaRN scaling must not touch the cache positions, so a distinct rope-position buffer
            // is required whenever scaling is active even if the rope delta is zero. The offset
            // below then degenerates to a copy (io_.rope_delta is the device copy of rope_delta_).
            ...
            if (rope_scaling_factor_ > 1.0F) {
                // A multimodal chunk carries its rope positions as a [len,3] MRoPE matrix rather
                // than the [len,1] text layout, and the ramp is elementwise over the whole buffer,
                // so flatten it. Both layouts are contiguous by construction (workspace_recipe
                // builds them via matrix(), which uses dense strides).
                Tensor rope_flat = rope_positions.view(
                    {rope_positions.ne[0] * rope_positions.ne[1] * rope_positions.ne[2] *
                     rope_positions.ne[3]});
                ops::scale_positions_yarn(
                    rope_flat, static_cast<std::uint32_t>(rope_scaling_original_context_),
                    rope_scaling_factor_, rope_flat, s);
            }
