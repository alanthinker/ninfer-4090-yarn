...
// Every execution context a schedule call builds must carry the YaRN rope-scaling policy, because
// the prefill path applies it on device. The members are therefore constructor-initialized rather
// than aggregate-initialized: an omitted policy would silently fall back to "scaling disabled" and
// leave prefill positioned differently from decode, so omission must fail loudly instead.
