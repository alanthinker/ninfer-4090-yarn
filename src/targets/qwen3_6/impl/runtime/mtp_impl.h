    ...
    return [&state, batch_size, k, envelopes] {
...

        {
            ...
            if (state.execution.rope_scaling_factor > 1.0F) {
                // The round kernel derives the AR rope positions from the raw frontier plus the
                // rope delta, so they need the same YaRN ramp the target path gets. Leaving them
                // unscaled would position the drafts differently from the verifier beyond the
                // native context, which collapses MTP acceptance exactly where the extension
                // matters. The whole frame is scaled because the kernel writes into a row-pitched
                // matrix whose stride is the frame's full batch: slots this round did not write
                // are stale and are gated out by ar_valid_columns, so scaling them is inert.
