...
void validate_target_options(DeviceContext& device, const EngineOptions& options) {
    ...
    case SpeculativeBackend::DFlash:
        ...
        // The DFlash proposal path derives its RoPE positions from the same tensor it uses as KV
        // cache positions (proposal_positions feeds both ops::rope and cache_positions), so a YaRN
        // ramp cannot be applied to one without corrupting the other. Refuse the combination
        // rather than silently positioning the drafts incorrectly.
        if (options.rope_scaling_factor > 1.0F) {
            throw std::invalid_argument(
                "YaRN rope scaling is not supported together with DFlash speculative decoding");
        }
        break;
    }
    ...
    // Upstream's NVFP4-valued KV modes need FP4 tensor cores; the sm_89 build carries their
    // kernels only as stubs (ops/nvfp4_sm86_stubs.cpp), so refuse them before any allocation.
