...
// The persistent expert kernels declare __launch_bounds__(..., 3), so a full residency wave is
// three CTAs per SM. Sizing the grid from the live SM count launches exactly that wave; a grid
// derived from any other part leaves a straggler wave that runs at a fraction of the width.
