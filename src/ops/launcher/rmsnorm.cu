...
// Past this many blocks the gated epilogue gives up its hoisted loads. Below roughly one block
// per SM the prefetch is the only source of overlap and those kernels run at 0.83x to 0.96x;
// above it a second resident block already supplies that overlap and only the register cost is
// left (35 -> 50 on the warp kernel), which measures 1.02x to 1.14x. Swept over grid size on
// both gated shapes the crossing sat near one live SM per block, so the threshold is the device
// SM count rather than a per-part literal.
...
void launch_rmsnorm(const Tensor& x, const Tensor& weight, const Tensor* z, Tensor& out,
    ...
    if (aligned2 && d == 128 && Epilogue == RmsEpilogue::Plain) {
        ...
        if constexpr (kGateOnGrid) {
            if (static_cast<std::int64_t>(blocks) > device_sm_count()) {
